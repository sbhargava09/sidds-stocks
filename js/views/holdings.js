...
// Range key → Yahoo Finance range string
...
export function renderHoldings(root) {
  ...
  const search = el('input', { type: 'search', placeholder: 'Search ticker or name…', value: '', autocomplete: 'off' });
  ...
  search.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowDown') { const first = dropdown.querySelector('.sd-item'); if (first) { first.focus(); e.preventDefault(); } }
  });
  document.addEventListener('pointerdown', function onOutside(e) {
    if (!searchWrap.contains(e.target)) { dropdown.hidden = true; document.removeEventListener('pointerdown', onOutside); }
  });
  ...
  [
    ['thesis', 'Group: thesis'], ['account', 'Group: account'], ['ticker', 'Sort: ticker'],
    ['name', 'Sort: name'], ['price', 'Sort: price'], ['gain', 'Sort: gain'],
    ['pct', 'Sort: % portfolio'], ['target', 'Sort: target distance'],
  ].forEach(([v, l]) => {
    const o = el('option', { value: v }, l);
...
// Resolve a CSS custom property from :root (canvas can't use CSS vars natively)
...
// ── Timestamp formatter for real Yahoo Finance timestamps ────────────────────
function formatTimestamp(epochSec, range) {
...
function defaultN(range) {
  return { '1D': 78, '5D': 65, '1M': 22, '6M': 26, 'YTD': 26, '1Y': 52 }[range] || 30;
}
...
function renderHoldingsList(container, enriched, total, f) {
  ...
  const sorters = {
    ...
    target: (a, b) => { const ad = targetDist(a), bd = targetDist(b); if (ad == null) return 1; if (bd == null) return -1; return ad - bd; },
  };
  if (f.sort === 'thesis' || f.sort === 'account') {
    const groupKey  = f.sort === 'thesis' ? (h => h.thesis_category || 'Market') : (h => h.account_type || 'Unassigned');
    const ORDER     = f.sort === 'thesis'
      ? ['Market', 'Tech', 'Dividend', 'Speculative']
      : ['Brokerage', 'Roth IRA', '401k', 'Unassigned'];
    const groups = {};
    list.forEach(h => { const key = groupKey(h); (groups[key] = groups[key] || []).push(h); });
    ...
    orderedKeys.forEach(key => {
      ...
      const group = el('div', { class: 'thesis-group' + (isCollapsed ? ' collapsed' : '') });
      group.dataset.thesis = key;

      const header = el('div', { class: 'thesis-header' });
      const headerLeft = el('div', { class: 'th-left' });
      headerLeft.appendChild(el('span', { class: 'name' }, `${key} · ${items.length}`));
      headerLeft.appendChild(el('span', { class: 'meta tabular' }, `${fmtMoney(groupValue, { compact: true })} · ${fmtPct(groupPct, { decimals: 1 })}`));
      header.appendChild(headerLeft);

      // Chevron SVG
      const chevron = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
      chevron.setAttribute('class', 'thesis-chevron');
      ...
      const body = el('div', { class: 'thesis-body' });
      items.forEach(h => body.appendChild(holdingCard(h, total)));
...
async function showSearchDropdown(query, dropdown, enriched, total, gen, getGen, searchInput) {
  ...
  if (holdingMatches.length) {
    ...
    holdingMatches.forEach(h => {
      ...
      btn.addEventListener('click', () => { state.ui.expandedTicker = h.ticker; dropdown.hidden = true; searchInput.value = ''; emit(); });
  ...
  if (liveQuote && !holdingMatches.find(h => h.ticker === liveQuote.ticker)) {
    ...
    btn.appendChild(right);
    btn.addEventListener('click', () => { dropdown.hidden = true; searchInput.value = ''; openAddHoldingModal(liveQuote.ticker, liveQuote.shortName || liveQuote.longName || ''); });
    dropdown.appendChild(btn);
  }
...
function holdingCard(h, total) {
  ...
  if (h.account_type) line1.appendChild(el('span', { class: 'h-tag watch' }, h.account_type));
  ...
  row.addEventListener('click', () => { state.ui.expandedTicker = expanded ? null : h.ticker; emit(); });
...
function editPanel(h) {
  ...
  delBtn.addEventListener('click', async (e) => {
    ...
    try { delBtn.disabled = true; await postAction('deleteHolding', { ticker: h.ticker }); toast('Deleted', 'success'); state.ui.expandedTicker = null; await loadAll(); }
    catch (err) { toast(err.message, 'error'); } finally { delBtn.disabled = false; }
  });
  ...
  saveBtn.addEventListener('click', async (e) => {
    ...
    try { saveBtn.disabled = true; saveBtn.textContent = 'Saving…'; await postAction('updateHolding', payload); toast('Saved', 'success'); await loadAll(); }
    catch (err) { toast(err.message, 'error'); } finally { saveBtn.disabled = false; saveBtn.textContent = 'Save'; }
  });
...
function openAddHoldingModal(prefillTicker = '', prefillName = '') {
  ...
  sec1.appendChild(grid1); form.appendChild(sec1);
  ...
  sec2.appendChild(grid2); form.appendChild(sec2);
  ...
  sec3.appendChild(grid3); form.appendChild(sec3);
  const actionsEl = el('div', { class: 'modal-actions' });
  actionsEl.appendChild(el('button', { class: 'btn secondary', onclick: closeModal }, 'Cancel'));
  ...
  save.addEventListener('click', async () => {
    ...
    try { save.disabled = true; save.textContent = 'Saving…'; await postAction('addHolding', payload); toast('Added', 'success'); closeModal(); await loadAll(); }
    catch (e) { toast(e.message, 'error'); save.disabled = false; save.textContent = 'Add holding'; }
  });
  actionsEl.appendChild(save); form.appendChild(actionsEl);
  openModal(form);
}
...
function openTxModal(ticker) {
  ...
  save.addEventListener('click', async () => {
    ...
    try { save.disabled = true; save.textContent = 'Saving…'; await postAction('addTransaction', { ticker, action: action.value, shares: shares.value, price_per_share: price.value }); toast('Transaction saved', 'success'); closeModal(); await loadAll(); }
...
export function openQuoteModal(quote, enriched, total) {
  ...
  const row  = (label, val, cls = '') => { const r = el('div', { class: 'modal-row' + (cls ? ' ' + cls : '') }); r.appendChild(el('label', {}, label)); r.appendChild(el('span', { class: 'tabular' }, val)); return r; };
  ...
  const actionsEl = el('div', { class: 'modal-actions' });
  actionsEl.appendChild(el('button', { class: 'btn secondary', onclick: closeModal }, 'Close'));
  actionsEl.appendChild(el('button', { class: 'btn', onclick: () => { closeModal(); openAddHoldingModal(quote.ticker, quote.shortName || quote.longName || ''); } }, 'Add to holdings'));
